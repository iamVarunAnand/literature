...
class GameManager {
   private:
    ...
    void CanPlayerBeRemoved(int pid) {
        if (players[pid].num_cards == 0) {
            ...
            for (int i = 0; i < kNumPlayers; ++i) {
                if (i != pid && players[i].isplaying == true) players[i].ReceivePlayerHasLeftGameUpdate(pid);
            }
        }
    }

   public:
    GameManager(int num_players = kNumPlayers) {
        dealer = Dealer();
    ...
    void PlayGame() {
        ...
        while (sets_declared < kNumTotalSets) {
            ...
            if (rcm.release) {
                ...
                // check if the player who gave the card can be removed from the
                // game
                ...
                if (dsm.declare) {
                    ...
                    // check if the player who declared the set can be removed
                    // from the game
            ...
            if (rcm.release && players[turn].isplaying == false) {
                ...
                if (sets_declared == kNumTotalSets) turn = -1;

                // turn goes to next player who is still playing
                else {
                    turn = (turn + 1) % kNumPlayers;
                    while (players[turn].isplaying == false) turn = (turn + 1) % kNumPlayers;
                }
            } else if (!rcm.release)
                turn = afcm.player_id;
